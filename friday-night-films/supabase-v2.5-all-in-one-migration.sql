...
-- V2.5 FIX: Archive changes require UPDATE permission on movies.
-- V2 originally only allowed SELECT / INSERT / DELETE, so the archive form
-- could appear to submit while Supabase rejected the status update.
drop policy if exists "public update movies" on public.movies;
create policy "public update movies" on public.movies
