...
let movies=[], votes=[], attendants=[], ratings=[], comments=[], nightSettings=null;
...
function currentAttendants(){return attendants.filter(a=>a.active!==false)}
function renderAttendants(){
 const current=currentAttendants();
 attendantsGrid.innerHTML='';
 attendeeCount.textContent=`${current.length} coming`;
 if(!current.length){attendantsGrid.innerHTML=`<div class="attendee-empty"><span>THE ROOM IS QUIET...</span><h3>Who's in?</h3><p>Claim a seat, pick an emoji, and leave a little note for the group.</p></div>`;return}
 current.sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)).forEach(a=>{
...
async function loadAll(show=true){if(!db){syncStatus.textContent='Supabase is not configured.';return}if(show)syncStatus.textContent='Syncing the ballot…';const [mr,vr,ar,rr,cr,sr]=await Promise.all([db.from('movies').select('*'),db.from('votes').select('*'),db.from('attendants').select('*'),db.from('ratings').select('*'),db.from('comments').select('*'),db.from('movie_night_settings').select('*').eq('id',1).maybeSingle()]);if(mr.error||vr.error||ar.error||rr.error||cr.error||sr.error){syncStatus.textContent=`Database setup needed: ${(mr.error||vr.error||ar.error||rr.error||cr.error||sr.error).message}`;return}movies=mr.data||[];votes=vr.data||[];attendants=ar.data||[];ratings=rr.data||[];comments=cr.data||[];nightSettings=sr.data||null;if(myAttendeeId&&!currentAttendants().some(a=>a.id===myAttendeeId)){localStorage.removeItem(ATTENDEE_KEY);myAttendeeId=''}syncStatus.textContent='';render();renderAttendants();renderArchive();renderGenreFilters();drawWheel();renderLineup();renderSavedPick();renderBallotDate()}
...
function renderBallotDate(){const raw=nightSettings?.current_ballot_date||currentBallotDateFallback();const d=new Date(raw+'T12:00:00');const today=new Date();const same=today.getFullYear()===d.getFullYear()&&today.getMonth()===d.getMonth()&&today.getDate()===d.getDate();document.querySelector('#date').textContent=`${same&&today.getDay()===5?'VOTE FOR TONIGHT':'VOTE FOR NEXT FRIDAY'} · ${d.toLocaleDateString('en-SG',{day:'2-digit',month:'long'}).toUpperCase()}`}
...
form.onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);if(!f.get('title')){status.textContent='Please search and select a movie first.';return}const payload={tmdb_id:+f.get('tmdb_id'),title:f.get('title'),release_year:+f.get('year')||null,poster_url:f.get('poster'),genres:f.get('genre')||'Film',submitted_by:f.get('name'),reason:f.get('reason'),status:'candidate',ballot_date:currentBallotDate()};status.textContent='Adding to the shared ballot…';const {error}=await db.from('movies').insert(payload);if(error){status.textContent=error.code==='23505'?'That movie is already on this week’s ballot.':`Could not add movie: ${error.message}`;return}e.target.reset();results.innerHTML='';status.textContent='';modal.close();await loadAll(false)};
...
async function submitArchive(e){
  ...
  try{
    ...
    archiveModal.close();
    await loadAll(false);
    document.querySelector('#archiveSection').scrollIntoView({behavior:'smooth'});openRollover(movieId);
  }catch(err){
...
attendeeForm.onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target);const payload={name:f.get('attendee_name').trim(),emoji:f.get('emoji'),note:f.get('note').trim()||null,active:true};const editId=e.target.dataset.editId;if(editId){const {error}=await db.from('attendants').update(payload).eq('id',editId);if(error){alert(`Could not update: ${error.message}`);return}}else{const {data,error}=await db.from('attendants').insert(payload).select('id').single();if(error){alert(`Could not join: ${error.message}`);return}myAttendeeId=data.id;localStorage.setItem(ATTENDEE_KEY,myAttendeeId)}resetAttendeeForm();attendeeModal.close();await loadAll(false)};

if(db){db.channel('fnf-live').on('postgres_changes',{event:'*',schema:'public',table:'movies'},()=>loadAll(false)).on('postgres_changes',{event:'*',schema:'public',table:'votes'},()=>loadAll(false)).on('postgres_changes',{event:'*',schema:'public',table:'attendants'},()=>loadAll(false)).on('postgres_changes',{event:'*',schema:'public',table:'ratings'},()=>loadAll(false)).on('postgres_changes',{event:'*',schema:'public',table:'comments'},()=>loadAll(false)).on('postgres_changes',{event:'*',schema:'public',table:'movie_night_settings'},()=>loadAll(false)).subscribe()}
loadAll();
...
function showWinner(m){if(!wheelWinner)return;wheelWinner.innerHTML=`<div class="winner-card">${m.poster_url?`<img src="${esc(m.poster_url)}" alt="">`:''}<div><p class="eyebrow">THIS WEEK'S PICK</p><strong>${esc(m.title)}</strong><small>${esc(m.release_year||'')} · ${esc(m.genres||'Film')} · ${voteCount(m.id)} vote${voteCount(m.id)===1?'':'s'}</small></div></div>`}
function renderSavedPick(){const id=nightSettings?.selected_movie_id;if(!id){if(wheelWinner&&!wheelSpinning)wheelWinner.innerHTML='';return}const m=movies.find(x=>x.id===id);if(m&&!wheelSpinning)showWinner(m)}
async function saveWeeklyPick(m){const base=nightSettings||{};const payload={id:1,start_time:base.start_time||null,location:base.location||null,food:base.food||null,group_note:base.group_note||null,selected_movie_id:m.id,current_ballot_date:currentBallotDate(),updated_at:new Date().toISOString()};const {error}=await db.from('movie_night_settings').upsert(payload,{onConflict:'id'});if(error){alert(`The wheel picked ${m.title}, but the pick could not be saved: ${error.message}`);return}nightSettings={...base,...payload};showWinner(m);renderLineup()}
function spinWheel(){if(wheelSpinning)return;const entries=wheelEntries();if(!entries.length){drawWheel();return}const winner=weightedPick(entries),target=winnerAngle(entries,winner);wheelSpinning=true;spinButton.disabled=true;wheelWinner.innerHTML='';const start=wheelRotation,startTime=performance.now(),duration=4200;const extra=Math.PI*2*(5+Math.floor(Math.random()*3));let delta=extra-target-start;while(delta<extra*.65)delta+=Math.PI*2;function frame(t){const p=Math.min(1,(t-startTime)/duration),ease=1-Math.pow(1-p,4);wheelRotation=start+delta*ease;drawWheel();if(p<1)requestAnimationFrame(frame);else{wheelRotation%=Math.PI*2;wheelSpinning=false;spinButton.disabled=false;saveWeeklyPick(winner.movie)}}requestAnimationFrame(frame)}
if(spinButton)spinButton.onclick=spinWheel;
window.addEventListener('resize',drawWheel);


// V3.1 weekly cycle + Tonight's Lineup
function currentBallotDateFallback(){const d=new Date(),days=(5-d.getDay()+7)%7;d.setDate(d.getDate()+days);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
function currentBallotDate(){return nightSettings?.current_ballot_date||currentBallotDateFallback()}
function nextFridayFrom(dateStr){const d=dateStr?new Date(dateStr+'T12:00:00'):new Date();d.setDate(d.getDate()+7);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
function renderLineup(){const card=document.querySelector('#lineupCard');if(!card)return;const s=nightSettings||{};const time=s.start_time?String(s.start_time).slice(0,5):'Time TBD';const loc=s.location||'Location TBD';const food=s.food||'Food TBD';const current=currentAttendants();const avatars=current.map(a=>esc(a.emoji)).join(' ');const pick=s.selected_movie_id?movies.find(m=>m.id===s.selected_movie_id):null;card.innerHTML=`<div><div class="lineup-kicker">TONIGHT'S LINEUP</div>${pick?`<div class="lineup-pick">${pick.poster_url?`<img src="${esc(pick.poster_url)}" alt="">`:''}<div><small>THIS WEEK'S PICK</small><strong>${esc(pick.title)}</strong><span>${esc(pick.release_year||'')} · ${esc(pick.genres||'Film')}</span></div></div>`:`<div class="lineup-no-pick">Not picked yet · Spin the reel when you're ready.</div>`}<div class="lineup-main"><strong>${esc(time)}</strong><span class="lineup-item">📍 ${esc(loc)}</span><span class="lineup-item">🍴 ${esc(food)}</span><span class="lineup-item">👧 ${current.length} coming</span></div>${s.group_note?`<p class="lineup-note">“${esc(s.group_note)}”</p>`:''}${avatars?`<div class="lineup-avatars">${avatars}</div>`:''}</div><div class="lineup-actions"><button id="editLineup" class="lineup-edit">Edit tonight →</button><button id="manualRollover" class="lineup-next">Start next week →</button></div>`;document.querySelector('#editLineup').onclick=openLineup;document.querySelector('#manualRollover').onclick=()=>openRollover(null)}
const lineupModal=document.querySelector('#lineupModal'),lineupForm=document.querySelector('#lineupForm');
function openLineup(){const s=nightSettings||{};lineupForm.elements.start_time.value=s.start_time?String(s.start_time).slice(0,5):'';lineupForm.elements.location.value=s.location||'';lineupForm.elements.food.value=s.food||'';lineupForm.elements.group_note.value=s.group_note||'';lineupModal.showModal()}
document.querySelector('#closeLineup').onclick=()=>lineupModal.close();
lineupForm.onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),payload={id:1,start_time:f.get('start_time')||null,location:f.get('location').trim()||null,food:f.get('food').trim()||null,group_note:f.get('group_note').trim()||null,selected_movie_id:nightSettings?.selected_movie_id||null,current_ballot_date:currentBallotDate(),updated_at:new Date().toISOString()};const {error}=await db.from('movie_night_settings').upsert(payload,{onConflict:'id'});if(error){alert(`Could not save tonight's lineup: ${error.message}`);return}lineupModal.close();await loadAll(false)};

const rolloverModal=document.querySelector('#rolloverModal'),rolloverForm=document.querySelector('#rolloverForm');
function openRollover(archivedId){const remaining=movies.filter(m=>m.status==='candidate'&&m.id!==archivedId);rolloverForm.dataset.archivedId=archivedId;document.querySelector('#rolloverCopy').innerHTML=`The watched film is now in the archive.<br><br><strong>${remaining.length} film${remaining.length===1?'':'s'}</strong> will carry over. Their votes will reset to 0, comments will stay, each film's waiting counter will go up by one, and tonight's attendees + lineup will reset.`;rolloverModal.showModal()}
document.querySelector('#closeRollover').onclick=()=>rolloverModal.close();document.querySelector('#skipRollover').onclick=()=>rolloverModal.close();
rolloverForm.onsubmit=async e=>{e.preventDefault();const btn=document.querySelector('#rolloverSubmit'),old=btn.textContent;btn.disabled=true;btn.textContent='ROLLING OVER…';try{const candidates=movies.filter(m=>m.status==='candidate');const nextDate=nextFridayFrom(currentBallotDate());for(const m of candidates){const {error}=await db.from('movies').update({weeks_waiting:Number(m.weeks_waiting||0)+1,ballot_date:nextDate}).eq('id',m.id);if(error)throw error}if(candidates.length){const ids=candidates.map(m=>m.id);const {error}=await db.from('votes').delete().in('movie_id',ids);if(error)throw error}const currentIds=currentAttendants().map(a=>a.id);if(currentIds.length){const {error}=await db.from('attendants').update({active:false}).in('id',currentIds);if(error)throw error}localStorage.removeItem(ATTENDEE_KEY);myAttendeeId='';const {error:settingsError}=await db.from('movie_night_settings').update({selected_movie_id:null,start_time:null,location:null,food:null,group_note:null,current_ballot_date:nextDate,updated_at:new Date().toISOString()}).eq('id',1);if(settingsError)throw settingsError;rolloverModal.close();await loadAll(false);document.querySelector('#attendantsSection').scrollIntoView({behavior:'smooth'})}catch(err){alert('Could not start next week: '+(err?.message||String(err)))}finally{btn.disabled=false;btn.textContent=old}};
