...
async function saveWeeklyPick(m){const base=nightSettings||{};const payload={id:1,start_time:base.start_time||null,location:base.location||null,food:base.food||null,group_note:base.group_note||null,selected_movie_id:m.id,updated_at:new Date().toISOString()};const {error}=await db.from('movie_night_settings').upsert(payload,{onConflict:'id'});if(error){alert(`The wheel picked ${m.title}, but the pick could not be saved: ${error.message}`);return}nightSettings={...base,...payload};showWinner(m);renderLineup()}
...
function currentBallotDate(){const d=new Date(),days=(5-d.getDay()+7)%7;d.setDate(d.getDate()+days);return `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`}
...
function renderLineup(){const card=document.querySelector('#lineupCard');if(!card)return;const s=nightSettings||{};const time=s.start_time?String(s.start_time).slice(0,5):'Time TBD';const loc=s.location||'Location TBD';const food=s.food||'Food TBD';const avatars=attendants.map(a=>esc(a.emoji)).join(' ');const pick=movies.find(m=>m.id===s.selected_movie_id);const pickHtml=pick?`<div class="lineup-pick">${pick.poster_url?`<img src="${esc(pick.poster_url)}" alt="${esc(pick.title)} poster">`:''}<div><span>THIS WEEK'S PICK</span><strong>${esc(pick.title)}</strong><small>${esc(pick.release_year||'')} · ${esc(pick.genres||'Film')}</small></div></div>`:`<div class="lineup-pick empty-pick"><div><span>THIS WEEK'S PICK</span><strong>Not picked yet</strong><small>Spin the reel when you're ready.</small></div></div>`;card.innerHTML=`<div class="lineup-content"><div class="lineup-kicker">TONIGHT'S LINEUP</div><div class="lineup-main"><strong>${esc(time)}</strong><span class="lineup-item">📍 ${esc(loc)}</span><span class="lineup-item">🍕 ${esc(food)}</span><span class="lineup-item">${attendants.length} coming</span></div>${s.group_note?`<p class="lineup-note">“${esc(s.group_note)}”</p>`:''}${avatars?`<div class="lineup-avatars">${avatars}</div>`:''}</div>${pickHtml}<button id="editLineup" class="lineup-edit">Edit tonight →</button>`;document.querySelector('#editLineup').onclick=openLineup}
...
rolloverForm.onsubmit=async e=>{e.preventDefault();const btn=document.querySelector('#rolloverSubmit'),old=btn.textContent;btn.disabled=true;btn.textContent='ROLLING OVER…';try{const candidates=movies.filter(m=>m.status==='candidate');const nextDate=nextFridayFrom(currentBallotDate());for(const m of candidates){const {error}=await db.from('movies').update({weeks_waiting:Number(m.weeks_waiting||0)+1,ballot_date:nextDate}).eq('id',m.id);if(error)throw error}if(candidates.length){const ids=candidates.map(m=>m.id);const {error}=await db.from('votes').delete().in('movie_id',ids);if(error)throw error}const {error:settingsError}=await db.from('movie_night_settings').update({selected_movie_id:null,updated_at:new Date().toISOString()}).eq('id',1);if(settingsError)throw settingsError;rolloverModal.close();await loadAll(false);document.querySelector('#moviesSection').scrollIntoView({behavior:'smooth'})}catch(err){alert('Could not start next week: '+(err?.message||String(err)))}finally{btn.disabled=false;btn.textContent=old}};
