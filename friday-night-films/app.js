...
lineupForm.onsubmit=async e=>{e.preventDefault();const f=new FormData(e.target),payload={id:1,start_time:f.get('start_time')||null,location:f.get('location').trim()||null,food:f.get('food').trim()||null,group_note:f.get('group_note').trim()||null,updated_at:new Date().toISOString()};const {error}=await db.from('movie_night_settings').upsert(payload,{onConflict:'id'});if(error){alert(`Could not save tonight's lineup: ${error.message}`);return}lineupModal.close();await loadAll(false)};
...
rolloverForm.onsubmit=async e=>{e.preventDefault();const btn=document.querySelector('#rolloverSubmit'),old=btn.textContent;btn.disabled=true;btn.textContent='ROLLING OVER…';try{const candidates=movies.filter(m=>m.status==='candidate');const nextDate=nextFridayFrom(currentBallotDate());for(const m of candidates){const {error}=await db.from('movies').update({weeks_waiting:Number(m.weeks_waiting||0)+1,ballot_date:nextDate}).eq('id',m.id);if(error)throw error}if(candidates.length){const ids=candidates.map(m=>m.id);const {error}=await db.from('votes').delete().in('movie_id',ids);if(error)throw error}rolloverModal.close();await loadAll(false);document.querySelector('#moviesSection').scrollIntoView({behavior:'smooth'})}catch(err){alert('Could not start next week: '+(err?.message||String(err)))}finally{btn.disabled=false;btn.textContent=old}};
