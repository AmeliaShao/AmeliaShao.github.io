...
let activeGenre='ALL', wheelGenre='ALL', wheelRotation=0, wheelSpinning=false;
...
function hasGenre(m,g){return g==='ALL'||String(m.genres||'Film').split('·').map(x=>x.trim()).includes(g)}
function renderGenreFilters(){const genres=['ALL',...genreList()];const a=document.querySelector('#genreFilters'),w=document.querySelector('#wheelGenreFilters');if(a){a.innerHTML=genres.map(g=>`<button class="genre-filter ${g===activeGenre?'active':''}" data-genre="${esc(g)}">${esc(g)}</button>`).join('');a.querySelectorAll('[data-genre]').forEach(b=>b.onclick=()=>{activeGenre=b.dataset.genre;render()})}if(w){if(!genres.includes(wheelGenre))wheelGenre='ALL';w.innerHTML=genres.map(g=>`<button class="genre-filter ${g===wheelGenre?'active':''}" data-wheel-genre="${esc(g)}">${esc(g)}</button>`).join('');w.querySelectorAll('[data-wheel-genre]').forEach(b=>b.onclick=()=>{wheelGenre=b.dataset.wheelGenre;renderGenreFilters();drawWheel()})}}
function render(){
 grid.innerHTML='';
 const allCandidates=movies.filter(m=>m.status==='candidate').sort((a,b)=>voteCount(b.id)-voteCount(a.id));
 const candidates=allCandidates.filter(m=>hasGenre(m,activeGenre));
 renderGenreFilters();
 if(!candidates.length){if(allCandidates.length){grid.innerHTML=`<div class="filter-empty">No ${esc(activeGenre)} films on this week's ballot.</div>`;return}grid.innerHTML=`<div class="empty"><span>NO NOMINATIONS YET</span><h3>Be the first to<br><em>pick a film.</em></h3><p>The ballot is empty. Add something brilliant, questionable, or both.</p><button id="emptyAdd">+ Suggest the first movie</button></div>`;document.querySelector('#emptyAdd').onclick=()=>modal.showModal();return}
...
function wheelMovies(){return movies.filter(m=>m.status==='candidate'&&hasGenre(m,wheelGenre))}
